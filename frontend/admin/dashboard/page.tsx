...
export default function AdminDashboardPage() {
  ...
  const fetchComplaints = () => { api.get("/complaints/").then(res => setComplaints(res.data)).catch(() => {}); };
  const updateStatus    = (id:number,status:string)  => { api.patch(`/complaints/${id}/`,{status}).then(()=>{fetchComplaints();showToast(`Status → "${status}"`);}).catch(()=>{}); };
  const updateRating    = (id:number,rating:number)  => { api.patch(`/complaints/${id}/`,{rating}).then(()=>{fetchComplaints();showToast("Rating updated");}).catch(()=>{}); };
  const deleteComplaint = (id:number)                => { api.delete(`/complaints/${id}/`).then(()=>{fetchComplaints();showToast("Deleted");setExpanded(null);}).catch(()=>{}); };
  ...
  return (
    <>
      ...
      <main className="main-content">
        <div className="page-header"><h1>Admin Dashboard</h1><p>Manage and respond to all student grievances</p></div>
